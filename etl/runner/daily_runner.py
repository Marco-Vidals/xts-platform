...
# ── Runner con retry ───────────────────────────────────────────────────────
def run_with_retry(fecha_ini: str | None = None, fecha_fin: str | None = None) -> bool:
    """
    Corre el ETL para el rango [fecha_ini, fecha_fin].
    Default: ayer -> hoy.
      - "ayer" captura RT/FMM completo del día anterior.
      - "hoy"  captura DA de hoy (publicado ayer en el DAM) y FMM parcial.
    ...
    if fecha_fin is None:
        fecha_fin = hoy   # incluye DA de hoy (publicado ayer en el DAM)
    ...
    for intento in range(1, MAX_RETRIES + 1):
        log.info(f"=== INTENTO {intento}/{MAX_RETRIES} — {fecha_ini} -> {fecha_fin} ===")

        resultados = run_all(fecha_ini, fecha_fin)
        fallidos   = [k for k, v in resultados.items() if not v]
        ...
        if not fallidos:
            ...
            status.update({
                "ultimo_ok":      hoy,
                "ultimo_fecha":   f"{fecha_ini} -> {fecha_fin}",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--desde", type=str, default=None,
                        help="Fecha inicio YYYY-MM-DD (default: ayer)")
    ...
    ok = run_with_retry(args.desde, args.hasta)
