...
from etl.caiso.caiso_api import (
    get_da_prices, get_fmm_prices, get_load, get_pml_cenace,
    _fetch_ren_fcst_csv, _ren_csv_to_hourly,
...
def extract_caiso(fecha_ini: str, fecha_fin: str) -> pd.DataFrame:
    ...
    print("[CAISO] Solar + Wind (SLD_REN_FCST)...")
    _ren_df = _fetch_ren_fcst_csv(fecha_ini, fecha_fin)
    df_solar = _ren_csv_to_hourly(_ren_df, "SOLAR", "solar_mw").rename(columns={"solar_mw": "SOLAR_CAISO"})
    df_wind  = _ren_csv_to_hourly(_ren_df, "WIND",  "wind_mw").rename(columns={"wind_mw":  "WIND_CAISO"})

    # Esqueleto horario
    horas = pd.date_range(fecha_ini, fecha_fin + " 23:00", freq="h")
    df = pd.DataFrame({"fecha": horas})

    for parte in [da_roa, da_tji, fmm_roa, fmm_tji, pml_ivy, pml_oms, df_load, df_solar, df_wind]:
...
def upsert_caiso(df: pd.DataFrame) -> None:
    ...
    caiso_cols = ["DA_ROA", "DA_TJI", "FMM_ROA", "FMM_TJI",
                  "PML_IVY", "PML_OMS", "LOAD_CAISO", "SOLAR_CAISO", "WIND_CAISO"]
    update_cols = [c for c in caiso_cols if c in df.columns]

    # Agregar columna WIND_CAISO si no existe en la tabla
    try:
        cursor.execute("""
            IF NOT EXISTS (
                SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME='DATOS_CAISO' AND COLUMN_NAME='WIND_CAISO'
            ) ALTER TABLE DATOS_CAISO ADD WIND_CAISO FLOAT
        """)
        conn.commit()
    except Exception as _e:
        print(f"[CAISO] No se pudo agregar WIND_CAISO: {_e}")
