...
def _fetch_oasis_xml(queryname: str, start_dt: str, end_dt: str,
    ...
    for attempt in range(4):
        resp = requests.get(OASIS_BASE, params=params, timeout=_TIMEOUT_OASIS)
...
def _lmp_components_to_hourly(df: pd.DataFrame) -> pd.DataFrame:
    """
    Extrae LMP + componentes (energy, congestion, loss) desde un OASIS DataFrame.
    DATA_ITEM values: LMP_PRC, LMP_ENE, LMP_CONG, LMP_LOSS
    Retorna df: fecha, lmp, energy, congestion, loss
    """
    if df.empty or "INTERVAL_START_GMT" not in df.columns:
        return pd.DataFrame()
    ...
    dfs = []
    for item, col in component_map.items():
        sub = df[df["DATA_ITEM"] == item][["fecha", "VALUE"]].rename(columns={"VALUE": col})
        sub = sub.groupby("fecha")[col].mean().reset_index()
        dfs.append(sub)
    ...
    result = dfs[0]
    for part in dfs[1:]:
        result = result.merge(part, on="fecha", how="outer")
...
def get_load(fecha_ini: str, fecha_fin: str) -> pd.DataFrame:
    """
    Carga horaria CAISO (SLD_FCST, DAM).
    ...
    df_all = pd.concat(frames, ignore_index=True)
    return _sld_to_hourly(df_all, "load_mw")


def _sld_to_hourly(df: pd.DataFrame, col: str) -> pd.DataFrame:
    """Convierte SLD_FCST a serie horaria."""
    if df.empty or "INTERVAL_START_GMT" not in df.columns:
    ...
    df["fecha"] = df["fecha"].dt.tz_convert("America/Los_Angeles").dt.tz_localize(None)
    df["fecha"] = df["fecha"].dt.floor("h")
    return df.groupby("fecha")[col].mean().reset_index()


# ── Solar CAISO ───────────────────────────────────────────────────────────────
def get_solar(fecha_ini: str, fecha_fin: str) -> pd.DataFrame:
    """
    Solar CAISO (SLD_REN_FCST, DAM).
    Retorna df: fecha, solar_mw.
    """
    ini_dt = datetime.strptime(fecha_ini, "%Y-%m-%d")
    fin_dt = datetime.strptime(fecha_fin, "%Y-%m-%d")
    ...
    df_all = pd.concat(frames, ignore_index=True)
    return _sld_to_hourly(df_all, "solar_mw")
